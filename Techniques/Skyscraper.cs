...
public class Skyscraper
{
    public bool Fill(List<int>[,] allCellCandidates, SudokuCell[,] cells)
    {
        ...
        // ======================
        // SÜTUN BAZLI (satır bazlı ile aynı mantık, satır/sütun rolleri değişmiş)
        // ======================
        for (int number = 1; number <= 9; number++)
        {
            // Her sütun için number adayının tam 2 satırda geçtiği sütunları topla
            List<(int col, int r1, int r2)> colsList = new();

            for (int col = 0; col < 9; col++)
            {
                List<int> rowsInCol = new();
                for (int row = 0; row < 9; row++)
                {
                    if (cells[row, col].IsFixed()) continue;
                    if (allCellCandidates[row, col].Contains(number))
                ...
                if (rowsInCol.Count == 2)
                    colsList.Add((col, rowsInCol[0], rowsInCol[1]));
            }

            // 2 sütun kombinasyonu
            for (int i = 0; i < colsList.Count; i++)
            {
                for (int j = i + 1; j < colsList.Count; j++)
                {
                    int col1 = colsList[i].col;
                    int col2 = colsList[j].col;

                    int a1 = colsList[i].r1;
                    int a2 = colsList[i].r2;
                    int b1 = colsList[j].r1;
                    int b2 = colsList[j].r2;
                    ...
                    if (a1 == b1) { common = a1; col1Other = a2; col2Other = b2; }
                    else if (a1 == b2) { common = a1; col1Other = a2; col2Other = b1; }
                    else if (a2 == b1) { common = a2; col1Other = a1; col2Other = b2; }
                    else if (a2 == b2) { common = a2; col1Other = a1; col2Other = b1; }

                    if (common == -1) continue;

                    // FIX: Ortak satırdaki iki hücre aynı blokta olmamalı.
                    // Aynı blokta olsalar bu Skyscraper değil, başka bir pattern'dir.
                    if (col1 / 3 == col2 / 3) continue;

                    // İki uç hücre (common satır dışındaki hücreler)
                    int p1r = col1Other, p1c = col1;
                    int p2r = col2Other, p2c = col2;
                    ...
                    for (int r = 0; r < 9; r++)
                    {
                        for (int c = 0; c < 9; c++)
                        {
                            ...
                            // Pattern hücrelerini atla
                            if ((r == col1Other && c == col1) ||
                                (r == col2Other && c == col2)) continue;
